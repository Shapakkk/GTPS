 ...
 * Supports:
 * - username: Find user by exact username (Default)
 * - /username: Find user by exact username (Legacy support)
...
export function parseUserTarget(
  ...
  // Legacy support: remove / if present, but also accept plain username
  if (target.startsWith("/")) {
    return { type: "name", value: target.slice(1) };
  }

  // Default: treat as username
  return { type: "name", value: target };
}