  ...
  public async execute(): Promise<void> {
    ...
    if (!parsedTarget) {
      this.peer.send(
        Variant.from(
          "OnConsoleMessage",
          "`4Invalid target format. Use `oUsername`4 or `o#id`4.``",
        ),
      );
      return;
    }
    ...
    if (targetPeer) {
      ...
      await targetPeer.saveToCache(); // PENTING: Update cache server
      await targetPeer.saveToDatabase(); // Update database

      this.peer.send(
        Variant.from(
          "OnConsoleMessage",
          `\`2Success! Added \`w${amount}\`2 gems to \`w${targetPeer.data.name}\`2.\n` +
          `\`o(Old: ${oldGems} -> New: ${targetPeer.data.gems})\`\``,
        ),
      );
      ...
      targetPeer.send(
        Variant.from(
          "OnConsoleMessage",
          `\`2You received \`w${amount}\`2 gems from Admin! Total: \`w${targetPeer.data.gems}\`\``,
        ),
      );
    } else if (targetData) {
      ...
      this.peer.send(
        Variant.from(
          "OnConsoleMessage",
          `\`2Success! Added \`w${amount}\`2 gems to \`w${targetData.name}\`2 (Offline).\n` +
          `\`o(Old: ${oldGems} -> New: ${newGems})\`\``,
        ),
      );
    }
  }
